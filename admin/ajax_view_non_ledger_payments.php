...
// ini_set('display_errors', 1); ini_set('display_startup_errors', 1); error_reporting(E_ALL);
  require 'includes/conn.php';
    ...
    $searchQuery = " ";
if($searchValue != ''){
   $searchQuery .= " and (non_customer_ledger_payments.id like '%".$searchValue."%' or
   customers.fname like '%".$searchValue."%' or
   non_customer_ledger_payments.id like '%".$searchValue."%' or
   non_customer_ledger_payments.reference_no like '%".$searchValue."%' or
   non_customer_ledger_payments.payment_date like '%".$searchValue."%' or
   non_customer_ledger_payments.total_shipments like '%".$searchValue."%' or
   non_customer_ledger_payments.total_delivered like '%".$searchValue."%' or
   non_customer_ledger_payments.total_returned like '%".$searchValue."%' or
   non_customer_ledger_payments.cod_amount like '%".$searchValue."%' or  
   non_customer_ledger_payments.delivery_charges like '%".$searchValue."%' or
   non_customer_ledger_payments.total_sell_flyers like '%".$searchValue."%' or
   non_customer_ledger_payments.gst_amount like '%".$searchValue."%')";
}
  if(isset($_POST['from']) && $_POST['to'])
  {
      $from = date('Y-m-d',strtotime($_POST['from']));
      $to = date('Y-m-d',strtotime($_POST['to']));
      $searchQuery .= " and DATE_FORMAT(non_customer_ledger_payments.payment_date, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(non_customer_ledger_payments.payment_date, '%Y-%m-%d') <= '".$to."' ";
  }
...
$sel = mysqli_query($con,"SELECT count(*) as allcount FROM non_customer_ledger_payments  WHERE 1 ");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];
## Total number of records with filtering

$sel = mysqli_query($con,"SELECT count(*) as allcount FROM non_customer_ledger_payments WHERE 1 ".$searchQuery."");
...
if($rowperpage == -1)
{
  $countQuery=mysqli_query($con,"SELECT COUNT(id) AS id FROM non_customer_ledger_payments WHERE 1 ".$searchQuery."");
...
$empQuery =  "SELECT non_customer_ledger_payments.*,customers.fname as customer,customers.bname as company_name FROM non_customer_ledger_payments LEFT JOIN  customers ON customers.id=non_customer_ledger_payments.customer_id WHERE 1  ".$searchQuery." ORDER by id desc  limit ".$row.",".$rowperpage;
  // echo $empQuery;
  // die;
// echo $empQuery;die();
    ...
    $shipments_html = $deliveries_html = $returned_html = $flyers_html = '';
                ...
                $action="";
                    if($fetch1['update_able'] == 1){
                     $action=" <a href='non_submit_bulk_ledger_payment.php?delete=".$fetch1['id']."&customer_id=".$fetch1['customer_id']."' style='color: #da1414;font-size: 14px;' class='fa fa-trash'></i></a>";
                    }

     $data[] = array(
       "srno"=>$sr_no,
       "customer_name"=>$fetch1['customer'] .'('.$fetch1['company_name'].')',
       "id"=>sprintf('%05d', $fetch1['id']),
       ...
       "delivery_charges"=> getConfig('currency').' '.number_format($fetch1['delivery_charges'], 2),
        ...
        "update_able"=>"<a  target='_blank' href='non_ledger_payment_view.php?payment_id=".$fetch1['id']."'class='fa fa-eye' style='font-size: 14px;'></i></a>".$action
    );
      $sr_no++;
  }
}
