                      ...
                      <div class="row">  
                          ...
                          ?>
                                ...
                                <optgroup label="<?= $fetch['accountGroupName']?>">
                                  <option value="<?= $fetch['id'] ?>"   <?php echo (isset($row['group_id']) && $row['group_id'] == $fetch['id'] ? 'selected' : ''); ?>  ><?= $fetch['accountGroupName']?></option>
                                  <?php
                                     while ($fetch1 = mysqli_fetch_array($child)) {
                                  ?>
                                  <option value="<?= $fetch1['id'] ?>"   <?php echo (isset($row['group_id']) && $row['group_id'] == $fetch1['id'] ? 'selected' : ''); ?>  ><?= $fetch1['accountGroupName']?></option>
                                   <?php
                                    }
                                  ?>
                                </optgroup>
             ...
             <form method="POST" action="">
              ...
              <div class="panel panel-primary">
                ...
                <div class="panel-body">
                  ...
                  <form method="POST" action="#">
					<?php
						$group2 = mysqli_query($con, "SELECT * FROM `tbl_grp_mapping`");
						while ($row1 = mysqli_fetch_array($group2)) {
                      ...
                      <div class="row">  
                          ...
                          <div class="col-sm-2">
                            <h5><?= $row1['description'] ; ?></h5>
                              <input type="hidden" class="form-control" required="" value="<?php echo   $row1['description']; ?>" placeholder="<?php echo getLange('name'); ?>" name="desc[]">
                          ...
                          <div class="col-sm-3">
                            <label>Account Group</label>
                               <select class="form-control select2 groupUnder" name="acc_grp[]" id="getLedger">
                                <?php
                                  while ($fetch = mysqli_fetch_array($parent)) {
                                 $child = mysqli_query($con, "SELECT * FROM `tbl_accountgroup` WHERE company_id IS NULL AND `groupUnder` = '".$fetch['id']."'");
                                ?>
