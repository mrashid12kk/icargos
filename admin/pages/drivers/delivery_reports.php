...
<div class="panel panel-default">
	<div class="panel-heading">Pickup Reports</div>
						...
						<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered dataTable no-footer" id="basic-datatable" role="grid" aria-describedby="basic-datatable_info"> 
							...
							<?php
								if(isset($_POST['generate'])){
									...
									$sql="Select * from users,deliver,orders where users.id=$driver_id and deliver.deliver_driver_id=$driver_id and orders.id=deliver.order_id and orders.order_date between '$from' and '$to' order by deliver.id desc";
								}
								else{
									$date=date('Y-m-d');
									$sql="Select * from users,deliver,orders where users.id=$driver_id and deliver.deliver_driver_id=$driver_id and orders.id=deliver.order_id  order by deliver.id desc";
								}
