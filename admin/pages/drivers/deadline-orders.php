<?php
	...
	if($method > -1) {
		...
		if($rowscount>0){
			echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">X</button><strong>Well done!</strong> you have successfully mark order as delivered .</div>';
		}
		else{
			echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">X</button><strong>Unsuccessful!</strong> You have not mark the order as delivered.</div>';
		}
	}
?>
<div class="panel panel-default">
	<div class="panel-heading">Deadline Orders</div>
						...
						<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered dataTable no-footer" id="basic-datatable" role="grid" aria-describedby="basic-datatable_info"> 
							<thead>
								<tr role="row">
								   <th class="sorting_asc" tabindex="0" aria-controls="basic-datatable" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="width: 179px;">Pickup Location</th>
								    ...
								    <th class="sorting" tabindex="0" aria-controls="basic-datatable" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 150px;">Delivery Date</th>
							...
							<tbody>
								<?php
									...
									$query1=mysqli_query($con,"Select * from deliver,orders where deliver.driver_id=".$fetch['id']." and orders.id=deliver.order_id and deliver.status='assigned' order by deliver.id desc") or die(mysqli_error($con));
									while($fetch1=mysqli_fetch_array($query1)){
										if($today >= strtotime(date($fetch1['delivery_date']))){
								?>
								...
								<tr class="gradeA odd" role="row">
									...
									<td><?php echo $fetch1['delivery_date']; ?></td>
									<td class="center">
