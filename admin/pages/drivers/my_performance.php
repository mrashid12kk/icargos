...
<div class="panel panel-default">
	<div class="panel-heading">Overall Performance</div>
						...
						<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered dataTable no-footer" id="basic-datatable" role="grid" aria-describedby="basic-datatable_info"> 
							<thead>
								<tr role="row">
								   ...
								   <th  tabindex="0" aria-controls="basic-datatable" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 150px;">Order Type</th>
								   <th  tabindex="0" aria-controls="basic-datatable" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 150px;">Delivery or Pickup Driver Name</th>
							...
							<?php
									...
									$date=date('Y-m-d');
									$date1=strtotime(date('Y-m-d').'-1 month');
									$last_month=date('Y-m-d',$date1);
									
									$sql="Select * from deliver,orders where (deliver.driver_id=$driver_id or deliver.deliver_driver_id=$driver_id) and orders.id=deliver.order_id  order by deliver.id desc ";
							...
							?>
								...
								<tr class="gradeA odd" role="row">
									...
									if($fetch1['driver_id']=="$driver_id"){
										echo "Pickup Order";
										$query22=mysqli_query($con,"Select * from users where id=".$fetch1['deliver_driver_id']);
										$fetch22=mysqli_fetch_array($query22);
										$driver_name="<b>Delivery Driver Name:</b>".$fetch22['Name'];
										
									}
									else{
										echo "Delivery Order";
										$query22=mysqli_query($con,"Select * from users where id=".$fetch1['driver_id']);
										$fetch22=mysqli_fetch_array($query22);
										$driver_name="<b>Pickup Driver Name:</b>".$fetch22['Name'];
									}
