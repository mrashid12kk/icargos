...
<div class="panel panel-default">
	<div class="panel-heading">Pending Delivery List</div>
				...
				<div class="row">
					<div class="col-sm-12 table-responsive">
						<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered dataTable no-footer" id="basic-datatable" role="grid" aria-describedby="basic-datatable_info"> 
							...
							<tbody>
								<?php
									$query1=mysqli_query($con,"Select * from deliver,orders where deliver.deliver_driver_id=".$fetch['id']." and orders.id=deliver.order_id and (deliver.status='in process' || deliver.status = 'accepted'|| deliver.status = 'postponed' || orders.status = 'delayed') order by deliver.id desc") or die(mysqli_error($con));
									while($fetch1=mysqli_fetch_array($query1)){
								?>
								<tr class="gradeA odd" role="row">
								<td><?php echo $fetch1['track_no']; ?></td>
									<td>
									<b>Date:</b> <?php echo $fetch1['delivery_date']; ?>
