...
$branches = mysqli_query($con, "SELECT * FROM branches WHERE 1 $destinationcity order by id desc ");
$branchselect = mysqli_query($con, "SELECT * FROM branches WHERE 1 order by id desc ");
$order_status = mysqli_query($con, "SELECT * FROM order_status");
?>

<div class="panel panel-default">

    <div class="panel-heading"><?php echo getLange('Branch Wise Report'); ?>
                    ...
                    <form method="POST" action="">
                        <div class="col-sm-2 left_right_none">
                            <div class="form-group">
                                ...
                                <label><?php echo getLange('Select Branch'); ?> </label>
                                <select class="form-control destination js-example-basic-single" name="destination">
                                    <option value="" <?php if ($active_destination == '') {
                                                            echo "selected";
                                                        } ?>>All</option>
                                    <?php while ($row = mysqli_fetch_array($branchselect)) { ?>
                                        <option value="<?php echo  $row['id']; ?>" <?php if ($active_destination == $row['id']) { echo"selected";  } ?>>
                                            <?php echo $row['name']; ?>
                                        </option>
                                    <?php } ?>
                                </select>
                        ...
                        <div class="col-sm-2 left_right_none">
                            <div class="form-group">
                                ...
                                <select class="form-control origin js-example-basic-single" name="status">
                                    <option value="" <?php if ($check_status == '') {
                                                            echo "selected";
                                                        } ?>>All</option>
                                    <?php while ($row = mysqli_fetch_array($order_status)) { ?>
                                        <option <?php if ($check_status == $row['status']) {
                                                    echo "selected";
                        ...
                        <div class="col-sm-2 left_right_none">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="date_type" id="date_type">
                                    <option value="order_date" <?php if (isset($date_type) && $date_type == 'order_date') {
                                                                    echo "Selected";
                                                                } ?>>Order Date</option>
                                    <option value="action_date" <?php if (isset($date_type) && $date_type == 'action_date') {
                                                                    echo "Selected";
                                                                } ?>>Status Date</option>
                                </select>
                ...
                <table class="table table-striped table-bordered dataTable_with_sorting no-footer orders_tbl">
                    ...
                    <tbody>
                        ...
                        while ($branch = mysqli_fetch_array($branches)) {
                            $query1 = mysqli_query($con, "SELECT SUM(collection_amount) as collection_amount,SUM(price) as price,SUM(pft_amount) as pft_amount,SUM(fuel_surcharge) as fuel_surcharge,SUM(grand_total_charges) as grand_total_charges,destination FROM orders WHERE 1 $searchQuery AND current_branch='" . $branch['id'] . "'");
