...
$voucher_type_query = "SELECT * FROM `tbl_vouchertype` WHERE voucherTypeName = 'Payment Voucher'";
...
$dn_query = "SELECT * FROM `tbl_payment_voucher_detail` ORDER BY id DESC LIMIT 1";
...
if(isset($_GET['voucher_no']))
{
	...
	$edit_query_debit_master = "SELECT pv_master.id as masterId,pv_master.description,pv_master.cheque_date,ledg.ledgerName,ledg.id as masterLedgerId FROM `tbl_payment_voucher_master` as pv_master LEFT JOIN tbl_accountledger as ledg ON pv_master.cash_bank_ledger_id = ledg.id  WHERE voucher_no = '{$voucher_no}'";

	$debit_edit_master_query_result = mysqli_query($con,$edit_query_debit_master);
	if($debit_edit_master_query_result->num_rows > 0)
	{
		$pv_edit_master_data = mysqli_fetch_assoc($debit_edit_master_query_result);
		$master_cheque_date = date("Y-m-d",strtotime($pv_edit_master_data['cheque_date']));
		$ledgerName = $pv_edit_master_data['ledgerName'];
		$ledgerId = $pv_edit_master_data['masterLedgerId'];
...
<form action="https://a.icargos.com/portal/admin/includes/custom_functions.php" method="POST" id="pr-form">
<div class="warper" id="transactions">
	<div class="panel panel-default">
		<div class="panel-heading">Payment Voucher</div>
		<?php
		...
		if(isset($pv_edit_master_data['masterId']))
		{
			$master_id = $pv_edit_master_data['masterId'];
		}
		...
		<div class="panel-body ">
			<div class="row">
				...
				<div class="col-sm-2 sidegap">
					...
					<select class="form-control cash-bank-ledger-name"  name="master_ledger_id">
						
						<?php 
							echo $ledgerId ? "<option value='{$ledgerId}' selected>{$ledgerName}</option>" 
				...
				<div class="col-sm-4 sidegap">
					<div class="row">
						...
						<div class="col-sm-4 searhbox">
							<button type="button" class="btn btn-primary" id="debit-search-btn" data-table="tbl_payment_voucher_detail" data-mastertable="tbl_payment_voucher_master" data-prtype="no">Search</button>
						</div>
					</div>
						...
						<tr class="table-item-row">
							...
							<td>
								<div class="action_btn">
									<button type="button" class="btn btn-info btn-sm add" data-vtype="no"><i class="fa fa-plus"></i></button>
								</div>
            ...
            <div class="row remarks_box">
            	<div class="col-sm-6 textarea_box sidegap">
					...
					<textarea class="form-control" name="description" id="remarks">
						<?php echo $pv_edit_master_data['description'] ?? ''; ?>	
					</textarea>
            	</div>
            	<div class="col-sm-6 sidegap">
            		<div class="input-right-box">
						<!-- <a name="import" class="btn btn-info import_file">Import</a> -->
						<button type="submit" name="payment_voucher_save" class="btn btn-info" id="debit_save">Save</button>
						<button type="submit" name="payment_voucher_save_print" value="save_print" class="btn btn-info">Save &amp; Print</button>
