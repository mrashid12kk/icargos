<?php
    ...
    {
        $branch_query = " AND current_branch = ".$_SESSION['branch_id'];
    }else
    {
        $branch_query = " AND current_branch = 1 ";
    }

	if (isset($_GET['type']) && !empty($_GET['type']) && $_GET['type']!='open') {
		$filter_query_status .=" AND status ='".$_GET['type']."' $branch_query";

	}
	if (isset($_GET['type']) && !empty($_GET['type']) && $_GET['type']=='open') {
		$filter_query_status .=" AND status !='Delivered' AND status != 'Returned to Shipper' $branch_query";
	}
	if(isset($_POST['submit'])){
		if(isset($_POST['tracking_no']) && !empty($_POST['tracking_no'])){
			...
			if (isset($_SESSION['branch_id']) && !empty($_SESSION['branch_id'])) {
            $query1 = mysqli_query($con,"SELECT * FROM orders WHERE DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."' $filter_query  $branch_query order by id desc ");
            }else{
            $query1 = mysqli_query($con,"SELECT * FROM orders WHERE DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."'   $filter_query  $branch_query order by id desc ");
            }

			$openCountRes = mysqli_fetch_array(mysqli_query($con, "SELECT count(id) as open_id from orders where status !='Delivered' AND status != 'Returned to Shipper' AND DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."' $filter_query $branch_query"));

		    $openCount = $openCountRes['open_id'];

		    $delCountRes = mysqli_fetch_array(mysqli_query($con, "SELECT count(id) as open_id from orders where status ='Delivered' AND DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."' $branch_query  $filter_query"));

		    $deliverCount = $delCountRes['open_id'];

		    $returnedCountRes = mysqli_fetch_array(mysqli_query($con, "SELECT count(id) as open_id from orders where  status = 'Returned to Shipper' AND DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."' $branch_query  $filter_query"));
	...
	elseif(isset($_GET['order_status']) && !empty($_GET['order_status'])){
		...
		if (isset($_SESSION['branch_id']) && !empty($_SESSION['branch_id'])) {
			$query1 = mysqli_query($con,"SELECT * FROM orders WHERE DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."'   $filter_query $filter_query_status $branch_query order by id desc ");


			$openCountRes = mysqli_fetch_array(mysqli_query($con, "SELECT count(id) as open_id from orders where status !='Delivered' AND status != 'Returned to Shipper' AND DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."'  $branch_query $filter_query"));
	    ...
	    $delCountRes = mysqli_fetch_array(mysqli_query($con, "SELECT count(id) as open_id from orders where status ='Delivered' AND DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."'   $filter_query $branch_query "));

	    $deliverCount = $delCountRes['open_id'];

	    $returnedCountRes = mysqli_fetch_array(mysqli_query($con, "SELECT count(id) as open_id from orders where  status = 'Returned to Shipper' AND DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."'   $filter_query $branch_query "));
				                            ...
				                            <div class="listing_boxes">
				                                <ul>
				                                    <li>
				                                        ...
				                                        <b><?php echo getLange('codamount'); ?>: <?php echo number_format((float)$fetch1['collection_amount'],2); ?></b>
				                                    </li>
				                                </ul>
				                            </div>
				                        </td>
