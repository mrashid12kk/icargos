...
if(isset($_POST['submit'])){
		...
		$query1 = mysqli_query($con,"SELECT * FROM orders WHERE DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."' AND status='picked' AND is_shipped=0 AND is_received=0 $active_customer_query order by id desc ");
		$pickup_query = mysqli_query($con,"SELECT * FROM zones WHERE city IN(SELECT origin FROM orders WHERE DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."' AND status='picked' AND is_shipped=0 AND is_received=0 $active_customer_query order by id desc) ");
		$delivery_query = mysqli_query($con,"SELECT * FROM zones WHERE city IN(SELECT destination FROM orders WHERE DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."' AND status='picked' AND is_shipped=0 AND is_received=0 $active_customer_query order by id desc) ");
	}else{
	...
	$query_status = mysqli_query($con,"SELECT * FROM orders WHERE (pickup_zone IS  NULL OR delivery_zone IS   NULL) AND status = 'picked' ");
...
<div class="panel panel-default">
	...
	<div class="panel-heading">Picked Orders</div>
			...
			<div id="basic-datatable_wrapper" class="dataTables_wrapper form-horizontal dt-bootstrap no-footer">
				...
				<div class="row">
					...
					<div class="col-sm-12 table-responsive gap-none">
				    	...
				    	<form method="POST" action="" id="bulk_submit">
				    		<input type="hidden" name="order_ids" id="print_data">
				    			<div class="col-sm-2 left_right_none">
									<div class="form-group">
										<label>Status</label>
										<select class="form-control order_sts" name="stat">
