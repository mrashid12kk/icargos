...
if(isset($_POST['submit']))
{
        ...
        $to = date('Y-m-d',strtotime($_POST['to']));
            // AND status='".$status."'
        $query = "SELECT * FROM assign_order_record WHERE DATE_FORMAT(`assign_data_time`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`assign_data_time`, '%Y-%m-%d') <= '".$to."'  AND assignment_type=2 AND assign_branch = '".$_SESSION['branch_id']."' $filter_query order by id desc ";
                        ...
                        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered dataTable no-footer" id="basic-datatable" role="grid" aria-describedby="basic-datatable_info">
                            ...
                            ?>
                                <tr class="gradeA odd" role="row">
                                    ...
                                    <td>
                                       <?php echo getBranchName($row_data['branch_id']); ?>
                                    </td>
