...
if (isset($_POST['get_origin_country']) && $_POST['get_origin_country'] == '1') {
    ...
    $queryx = mysqli_query($con, "SELECT * FROM country where `country_name`= '".$customer_country."'");
    $x = mysqli_fetch_array($queryx);
    // var_dump();
    // die();
    ...
    if ($rowcount > 0) {
        while ($row = mysqli_fetch_array($query)) {
            // var_dump($row);
            if(isset($customer_country) && !empty($customer_country)){
                $selected= $row['id'] == $x['id'] ? 'selected' : '';
            }
            else{
                $selected = $row['country_name'] == $country_default['value'] ? 'selected' : '' ;
            }
...
if (isset($_POST['get_origin_state']) && $_POST['get_origin_state'] == '1') {   
    ...
    $customer_country = $customer_data['origin'];
    // var_dump($customer_country);
    $queryx = mysqli_query($con, "SELECT * FROM state WHERE state_name = '".$customer_state."'");
    $x = mysqli_fetch_array($queryx);
    ...
    if ($rowcount > 0) {
        while ($row = mysqli_fetch_array($query)) {
            $selected=isset($customer_state) && $row['id'] == $x['id'] ? 'selected' : '';
            // var_dump( $row);
...
if (isset($_POST['get_origin_city']) && $_POST['get_origin_city'] == '1') {
    ...
    $search_city = '';
    $queryx = mysqli_query($con, "SELECT * FROM cities WHERE city_name = '".$customer_city."'");
    $x = mysqli_fetch_array($queryx);
    ...
    if ($rowcount > 0) {
        while ($row = mysqli_fetch_array($query)) {
             if(isset($customer_city) && !empty($customer_city)){
                $selected= $row['id'] == $x['id'] ? 'selected' : '';
            }
            else{
                $selected = $row['city_name'] == $city_default['value'] ? 'selected' : '' ;
            }
...
$active_customer_id = $_POST['active_customer_id'];
    ...
    if ($rowcount > 0) {
        while ($row = mysqli_fetch_array($query)) {
            ...
            $destination_country .= "<option ".$selected.">" . $row['country_name'] . "</option>";
        }
    }
    if (isset($destination_country) && $destination_country != '') {
        echo $destination_country;
        exit();
    }
}
