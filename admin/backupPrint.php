...
.icargo_box ul li h1,.icargo_box ul li h3{
    margin: 0;
}
...
.list_fix:first-child, .table_gap:first-child {
    margin-right: 3px;
}
.d_flex_cus .col_cus {
    width: 49%;
...
.main_address_2 .dot_line ,.main_address_3 .dot_line,.main_address_44 .dot_line{
...
.table_gap tfoot, .table_gap thead {
    background: #f5f5f5;
}
@media print {
    .icargo_box ul li:nth-child(3) h3 {
...
if(isset($fetch) && !empty($fetch))
{
 ...
 ?>
  <div class="single_page">
    ...
    <div class="main_div_full" style="padding: 74px 0 0;">
        <div class="full_row">
          ...
          <div class="col_2 input_fix">
              <label>Date#</label>
              <input type="text" value="<?php echo date('Y-m-d' , strtotime($fetch['dates'])) ?>" readonly>
          </div>
          <div class="col_2 input_fix">
              <label>Region#</label>
    ...
    <div class="main_address">
        <div class="full_row">
          ...
          <div class="col_12 dot_line">
           ...
           <div class="col_12 dot_line">
            <b>Pickup Address :</b>
            <p><span><?php echo isset($fetch['billing_address']) ? $fetch['billing_address'] : ''; ?></span></p>
          </div>
        </div>
   ...
   <div class="main_address main_address_2">
       <div class="full_row">
             <div class="col_6 left_box">
                  ...
                  <div class="dot_line">
                    <b>Contact Person :</b>
                    <p><span><?php echo isset($fetch['contact_person']) ? $fetch['contact_person'] : ''; ?></span></p>
                  </div>
                  <div class="dot_line">
                    <b>Designation :</b>
              ...
              <div class="col_6 right_box">
                <div class="dot_line">
                    <b>Inductry Code :</b>
                    <p><span><?php echo isset($fetch['industry_code']) ? $fetch['industry_code'] : ''; ?></span></p>
                  </div>
                  <div class="dot_line">
                    <b>GST :</b>
 ...
 <div class="main_address main_address_3">
      ...
      <div class="dot_line">
        ...
        <p><span><?php echo isset($fetch['billing_instruction']) ? $fetch['billing_instruction'] : ''; ?></span></p>
      </div>
      <div class="dot_line">
        <b>Monthly Shipment :</b>
        <p><span><?php echo isset($fetch['expected_shipment']) ? $fetch['expected_shipment'] : ''; ?></span></p>
      </div>
      <div class="dot_line">
        <b>Fuel Adj Charges  :</b>
        <p><span><?php echo isset($fetch['fuel_formula']) ? $fetch['fuel_formula'].'%' : ''; ?></span></p>
      </div>
      <div class="dot_line">
        <b>Handling Charges :</b>
        <p><span><?php echo isset($fetch['handling_charges']) ? $fetch['handling_charges'] : ''; ?></span></p>
      </div>
      </div>
  <div class="col_6 right_box">
    <div class="dot_line">
        <b>Associated Companies :</b>
        <p><span><?php echo isset($fetch['assosciated_company']) ? $fetch['assosciated_company'] : ''; ?></span></p>
      </div>
      <div class="dot_line">
        <b>Yearly Tariff Increase:</b>
        <p><span><?php echo isset($fetch['tariff_increase']) ? $fetch['tariff_increase'] : ''; ?></span></p>
      </div>
      <div class="dot_line">
        <b>Payemnt Within :</b>
        <p><span><?php echo isset($fetch['payment_within']) ? $fetch['payment_within'] : ''; ?></span></p>
      </div>
      <div class="dot_line">
        <b>Monthly Revenue :</b>
        <p><span><?php echo isset($fetch['monthly_revenue']) ? $fetch['monthly_revenue'] : ''; ?></span></p>
      </div>
      <div class="dot_line">
        <b>Flexible Fule Formula :</b>
        <p><span><?php echo isset($fetch['fuel_formula']) && $fetch['fuel_formula']==1 ? 'Yes' : 'No'; ?></span></p>
      </div>
      <div class="dot_line">
        <b>Other Charges :</b>
        <p><span><?php echo isset($fetch['other_charges']) ? $fetch['other_charges'] : ''; ?></span></p>
      </div>
  </div>
    </div>
</div>
...
<div class="main_address_box height_fix d_flex_cus">
   <?php 
    $customer_tarif_q=mysqli_query($con,"SELECT * FROM customer_tariff_detail WHERE customer_id=".$_GET['account_no']." GROUP BY tariff_id");
    $rowscount=mysqli_affected_rows($con);
    if ($rowscount> 0) {  
     while ($customer_tariff=mysqli_fetch_array($customer_tarif_q)) {
        $tarif_q=mysqli_fetch_array(mysqli_query($con,"SELECT * FROM tariff WHERE id=".$customer_tariff['tariff_id']));
        $service_type_q=mysqli_fetch_array(mysqli_query($con,"SELECT service_type FROM services WHERE id=".$tarif_q['service_type']));
        $product_name_q=mysqli_query($con,"SELECT * FROM products WHERE id=".$tarif_q['product_id']);
        $tariff_name=isset($tarif_q['tariff_name']) ? $tarif_q['tariff_name'] : '';
        $service_type=isset($service_type_q['service_type']) ? $service_type_q['service_type'] :'';
        $product_name=isset($product_name_q['name']) ? $product_name_q['name'] :'';
        $rowscount=mysqli_affected_rows($con);
         if ($rowscount> 0) {
          $product_name=mysqli_fetch_assoc($product_name_q);
        ?>
      <div class="col_cus table_gap">
      <h4>Product: <?php echo isset($product_name) ? $product_name : ''; ?></h4>
      <table>
         <thead>
            <tr>
               <th>Tarrif Type</th>
         ...
         <tbody>
          <?php $product_q=mysqli_query($con,"SELECT * FROM product_type_prices WHERE product_id=".$product_id);
           $rowscount=mysqli_affected_rows($con);
            if ($rowscount> 0) {
              while ($product=mysqli_fetch_array($product_q)) {
               ?>
                <tr>
                   <th><?php echo $tariff_name; ?></th>
                   <td><?php echo $product['start_range']; ?></td>
                   <td><?php echo $product['end_range']; ?></td>
                   <td><?php echo $product['division_factor']; ?></td>
       ...
       $result_q = mysqli_query($con, "SELECT * FROM pay_mode WHERE account_type=".$fetch['customer_type']);
      $rowscount=mysqli_affected_rows($con);
      if ($rowscount > 0)
      {
        while ($result=mysqli_fetch_assoc($result_q)) {
          $result_q = mysqli_query($con, "SELECT * FROM tariff WHERE pay_mode='".$result['id']."'");
          $rowscount=mysqli_affected_rows($con);
          if ($rowscount > 0)
          {
            while ($result=mysqli_fetch_array($result_q)) {
               $tariff_name=$result['tariff_name'];
                $product_id=$result['product_id'];
                $service_type=$result['service_type'];
                $service_type_q=mysqli_fetch_array(mysqli_query($con,"SELECT service_type FROM services WHERE id=".$service_type));
                $service_type=$service_type_q['service_type'] ? $service_type_q['service_type'] : '';
              $product_name_q=mysqli_query($con,"SELECT * FROM products WHERE id=".$product_id);
              $rowscount=mysqli_affected_rows($con);
              if ($rowscount> 0) {
                $product_name=mysqli_fetch_assoc($product_name_q);
                ?>
                  <div class="col_cus table_gap">
                  <h4>Product: <?php echo isset($product_name['name']) ? $product_name['name'] : ''; ?></h4>
                  <table>
                     <thead>
                        <tr>
                           <th>Tarrif Type</th>
                     ...
                     <tbody>
                      <?php $product_q=mysqli_query($con,"SELECT * FROM product_type_prices WHERE product_id=".$product_id." ORDER BY start_range ASC");
                       $rowscount=mysqli_affected_rows($con);
                        if ($rowscount> 0) {
                          while ($product=mysqli_fetch_array($product_q)) {
                            $start_range=$product['start_range'];
  ...
  <div class="main_address main_address_box">
      <div class="full_row ">
        ...
        <p><span><?php echo isset($fetch['special_instruction']) ? $fetch['special_instruction'] : ''; ?></span></p>
      </div>
      <div class="col_12 dot_line">
        <b> Frequent Destinations :</b>
        <p><span><?php echo isset($fetch['frequent_destination']) ? $fetch['frequent_destination'] : ''; ?></span></p>
      </div>
  </div>
  </div>
