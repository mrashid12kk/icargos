<?php
   function email_admin_booking($id,$orde_id){
    global $con;
        ...
        // $email_data = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM email_templates WHERE sms_events='Pickup Request' "));
        ...
        // $email_data = mysqli_fetch_array(mysqli_query($con,"SELECT * FROM email_templates WHERE sms_events='Delivered' "));
        $template_content='';
         if (!empty($email_data)) {
        ...
        if (in_array(1, $prev_array)) {
           $data['email'] = $_SESSION['users_email'];
             $insert_qry="INSERT INTO `email_detail`(`contact_email`,`message`,`created_on`,`status`,`template_id`,`company_id`,`subject`,`order_status`) VALUES ('". $_SESSION['users_email']."','".$template_content."','".$date."','1','".$email_data['id']."','". $_SESSION['users_id']."','Admin','".getKeyWord($order_data['status'])."') ";
        ...
        if (in_array(2, $prev_array)) {
             ...
             $insert_qry="INSERT INTO `email_detail`(`contact_email`,`message`,`created_on`,`status`,`template_id`,`company_id`,`subject`,`order_status`) VALUES ('".$order_data['semail']."','".$template_content."','".$date."','1','".$email_data['id']."','".$order_data['customer_id']."','Shipper','".$customerstatuslang."') ";
             $query=mysqli_query($con,$insert_qry);
            $message['subject'] = $customerstatuslang;
        ...
        if (in_array(3, $prev_array)) {
         $data['email'] = $order_data['remail'];
             $insert_qry="INSERT INTO `email_detail`(`contact_email`,`message`,`created_on`,`status`,`template_id`,`subject`,`order_status`) VALUES ('".$order_data['semail']."','".$template_content."','".$date."','1','".$email_data['id']."','Consignee','".getKeyWord($order_data['status'])."') ";
             $query=mysqli_query($con,$insert_qry);
            $message['subject'] = 'Order Booked';
