...
error_reporting(E_ALL);
	...
	if(isset($_POST['delete_track']) && $_POST['delete_track']){
	
		$track_no = $_POST['cn_no'];
		$manifest_no = $_POST['manifest_no'];
		$q = "DELETE FROM order_logs where order_no = '".$track_no."' and trans_number ='".$manifest_no."' ";
		$sql = mysqli_query($con,$q );

		if(true){
			$getrows =mysqli_query($con,"SELECT `order_status` FROM `order_logs` WHERE `order_no` = '".$track_no."' order by id DESC LIMIT 1");
			$status = mysqli_fetch_array($getrows);
			$current_status = $status['order_status'];
			if(true){
				$update_query = mysqli_query($con, "UPDATE orders set status = '".$current_status."' where track_no = '".$track_no."'");
				if(true){
					$sql = mysqli_query($con, "DELETE FROM manifest_detail where track_no = '".$track_no."' and manifest_no ='".$manifest_no."' ");
					if(true){
						$sql1 = mysqli_query($con, "SELECT * FROM manifest_detail where manifest_no ='".$manifest_no."' ");
						if(mysqli_affected_rows($con) == 0){
							$sql2 = mysqli_query($con, "DELETE FROM manifest_master where manifest_no ='".$manifest_no."' ");
							echo '1';
						}
						else{
							echo '2';
