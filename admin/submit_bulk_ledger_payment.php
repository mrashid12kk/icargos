...
function insertToLedgerTable($connection, $table, $data)
{
    ...
    $prev_balance_history =  (isset($data['prev_balance_history']) && $data['prev_balance_history']) ? $data['prev_balance_history'] : 0;
...
if (isset($_POST['submit'])) {
    ...
    $insert_data = [
        ...
        'total_returned_fee' => formated_value($_POST['total_return_fee']),
    ...
    if (insertToLedgerTable($con, 'customer_ledger_payments', $insert_data)) {
        ...
        if ($id) {
            ...
            mysqli_query($con, "UPDATE customer_ledger_payments SET update_able = 0 WHERE customer_id = $customer_id");
            mysqli_query($con, "UPDATE customer_ledger_payments SET update_able = 1 WHERE id = $id");
            // $url = 'https://pla.a.eloerp.net/accounting/ledger_posting_api/ledger_posting';
            // $ch = curl_init();
    ...
    $order = mysqli_query($con, "SELECT * FROM customer_ledger_payments WHERE id = " . $_GET['delete']);
    ...
    if ($order) {
        ...
        mysqli_query($con, "DELETE FROM customer_ledger_payments WHERE id = " . $order->id);
        mysqli_query($con, "UPDATE customer_ledger_payments SET update_able = 1 WHERE customer_id = " . $customer_id . " ORDER BY id DESC LIMIT 1");
    }
    header('Location: ' . $_SERVER['HTTP_REFERER']);
    exit();
}