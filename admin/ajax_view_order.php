...
$tracking_no_bulk = $_POST['tracking_no_bulk'];
...
while ($fetch1 = mysqli_fetch_assoc($empRecords)) {
  $delayed='';
  if ($fetch1['status']!='cancelled' && $fetch1['status']!='New Booked' && $fetch1['status']!='Delivered' && $fetch1['status']!='Returned to Shipper'){ 
    $action_date=strtotime($fetch1['action_date']);
    $date=strtotime(date('Y-m-d H:i:s'));
    $diff = $date - $action_date;
    $hours = $diff / ( 60 * 60 );
    $order_delat_time=getConfig('order_delayed_time');
    if($hours>=$order_delat_time){
      $delayed_v ="<td class='price-box'><div class='ribbon'><span>DELAYED</span></div>  </td>";
    }
  } 
  $order_booking_type='';
  ...
  $data[] = array(
   ...
   "payment"=>"".($fetch1['payment_status']=='Paid' ? "<td class='price-box'><div class='ribbon'><span>".getLange('paid')."</span></div>  </td>" : $delayed_v.$order_booking_type)."",
 );
  $sr_no ++;
}
