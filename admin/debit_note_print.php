...
if(isset($_GET['voucher_number']))
{
    ...
    $master_query = "SELECT master.voucher_no,master.id,master.created_on,master.description,users.Name,users.user_name FROM `tbl_debit_note_master` as master LEFT JOIN users ON master.user_id = users.id  WHERE master.voucher_no = '{$voucherNumber}'";
    ...
    if($master_query_result->num_rows > 0)
    {
        ...
        $detail_query = "SELECT detail.narration,detail.chequeNo,detail.id,detail.amount,ledger.ledgerName,detail.type ,CONCAT(detail.amount,'-',detail.type) as  amount_value, DATE_FORMAT(detail.chequeDate,'%b-%d-%Y') as cheque_date FROM `tbl_debit_note_detail` as detail LEFT JOIN tbl_accountledger as ledger ON detail.ledgerId = ledger.id WHERE detail.master_id = ".$master_data['id'];
...
.table-box th{
    ...
    <div class="print_wrap">
        ...
        <div class="head_info_box">
            <h1 class="text-center bt-1">Debit Note</h1>
        </div>
    ...
    ?>
        ...
        <script type="text/javascript">
            ...
            function closePrintView() {
                document.location.href = 'https://a.icargos.com/portal/admin/debit_note.php';
            }
        </script>
    <?php
