...
if( isset($_GET['getriders']) ){
	$pickup_rider = $_GET['rider_id'];
	$is_assigned = $_GET['is_assigned'];
	if($is_assigned !=""){
		if($is_assigned == 2){
			$assigned_check = " AND pickup_rider IS NULL ";
		}elseif($is_assigned == 1){
			$assigned_check = " AND pickup_rider != '' ";
		}
	}
	$pickup_rider_search = " AND pickup_zone IN( SELECT id FROM zones WHERE riders='".$pickup_rider."' ) $assigned_check ";
...
if(isset($_GET['submit'])){
		...
		$query1 = mysqli_query($con,"SELECT * FROM orders WHERE DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."' $origin_check $destination_check $status_check $pickup_rider_search $active_customer_query order by id desc ");
	}else{
		$from = date('Y-m-d', strtotime('today - 30 days'));
		$to = date('Y-m-d');

		$query1 = mysqli_query($con,"SELECT * FROM orders WHERE DATE_FORMAT(`order_date`, '%Y-%m-%d') >= '".$from."' AND  DATE_FORMAT(`order_date`, '%Y-%m-%d') <= '".$to."' $pickup_rider_search $active_customer_query  order by id desc ");
	}
	////////////////////////////////////////////////////////////////////
	...
	$rider_query = mysqli_query($con,"SELECT * FROM users WHERE id IN(".$rider_imp.")  ");
	///////////////////////////////////////////////////////////////////
 ?>
 <?php 
 if(!isset($_GET['print'])){ ?>
 <div class="row">
    <a target="_blank" style="margin-left: 0;margin-bottom: 10px;" href="<?php echo 'pickup_report.php?'.http_build_query(array_merge($_GET, ['print' => 1])); ?>"  class="btn btn-info" >Print</a>
</div>
<?php } ?>
<?php 
if(!isset($_GET['print'])){
 ...
 <div id="same_form_layout">
	...
	<form method="GET" action="" >
		...
		<div class="col-sm-2 left_right_none">
			...
			<select name="rider_id"   class="form-control js-example-basic-single pickup_rider rider_id">
				<option value="1" selected disabled>Select Pickup Rider</option>
...
<div class="panel panel-default" style="margin-top:0;">
	<?php if(!isset($_GET['print'])){ ?>
	<div class="panel-heading shipment_report order_box" >Pickup Report
...
if(!isset($_GET['print'])){
 ...
 <form method="GET" action="">
    ...
    padding: 8px 4px;text-align:center;" class="sorting" tabindex="0" aria-controls="basic-datatable" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 244px;">COD Amount</th>
     <th style="  font-size: 11px !important;
    padding: 8px 4px;text-align:center;" class="sorting" tabindex="0" aria-controls="basic-datatable" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 244px;">Pickup Zone</th>
      <th style="  font-size: 11px !important;
    padding: 8px 4px;text-align:center;" class="sorting" tabindex="0" aria-controls="basic-datatable" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 244px;">Pickup Rider</th>
    ...
    padding: 5px 7px;text-align:left;"></td>
			...
			<a  href="#" class="assign_rider btn btn-info">Assign Pickup Rider</a>
		</div>
	<?php } ?>
				</div>
			</div>
