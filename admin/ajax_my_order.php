...
$searchQuery = " ";
    if (isset($_SESSION['user_role_id']) && $_SESSION['user_role_id']== getfranchisemanagerId()) {
        $searchQuery .="AND user_id=".$_SESSION['users_id'];
    }
 if(isset($_SESSION['branch_id']) && !empty($_SESSION['branch_id']))
    {
        $searchQuery .= " AND current_branch = ".$_SESSION['branch_id'];
    }else
    {
...
if($searchValue != ''){
   ...
   $searchQuery .= " and (track_no like '%".$searchValue."%' or 
     ...
     origin like '%".$searchValue."%' or 
      ...
      sphone like'%".$searchValue."%' or 

      semail like'%".$searchValue."%' or 

      sender_address like'%".$searchValue."%' or 

      order_type like'%".$searchValue."%' or 

      net_amount like'%".$searchValue."%' or 

      rname like'%".$searchValue."%' or 

      rphone like'%".$searchValue."%' or 

      receiver_address like'%".$searchValue."%' or 

      collection_amount like'%".$searchValue."%' or 

      payment_status like'%".$searchValue."%' or 
...
$sel = mysqli_query($con,"select count(*) as allcount from orders WHERE 1 ".$searchQuery);
...
while ($fetch1 = mysqli_fetch_assoc($empRecords)) {
  ...
  // die();
   ...
   $data[] = array(
     ...
     "id"=>"<td class='all_brands'><div class='brand_logo'><img src='".BASE_URL."".customer_image($fetch1['customer_id'])."'></div></td>",
