...
if($searchValue != ''){
   ...
   $searchQuery .= " and (email_templates.id like '%".$searchValue."%' or

      email_templates.template_code like '%".$searchValue."%' or

      email_templates.template_name like '%".$searchValue."%' or

      email_templates.created_on like '%".$searchValue."%' or

      email_templates.status like '%".$searchValue."%' or

      email_templates.template_content like '%".$searchValue."%' or
     
      email_templates.sms_events like '%".$searchValue."%'  ) ";
}

## Total number of records without filtering
$sel = mysqli_query($con,"SELECT count(*) as allcount FROM email_templates");
...
// die;
$sel = mysqli_query($con,"SELECT count(*) as allcount FROM email_templates WHERE 1 ".$searchQuery."");
...
$empQuery = "SELECT * FROM email_templates WHERE 1 ".$searchQuery." order by ".$columnName."  ".$columnSortOrder." limit ".$row.",".$rowperpage;
// echo $empQuery;
// die;
...
while ($fetch1 = mysqli_fetch_assoc($empRecords)) {
  ...
  $send_to='';
   ...
   $data[] = array(
      ...
      "status"=>$sts,
      "action"=>'<a href="template_form_email.php?id= '.$fetch1['id'].'" class=""><i class="fa fa-edit"></i></a>
        <a href="delete_sms.php?template_email= '.$fetch1['id'].'"  onclick="return confirm(' . "'Are you sure you want to Delete?'" . '); return false"><i class="fa fa-trash"></i></a>'
   );
   // $sr_no ++;
}
## Response
