...
if (isset($_SESSION['users_id']) && ($_SESSION['type'] !== 'driver' && $_SESSION['type'] == 'admin')){
    ...
    include "includes/header_bkp.php";
    ?>
    <style>
        a {
    ...
    <body data-ng-app>
        ...
        <section class="content">
            ...
            <div class="warper container-fluid">
                ...
                <?php
                include "pages/booking/booking_form.php";
                ?>
            </div>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response) {
                            ...
                            var api_msg = response.api_msg;
                            var api_msg_status = response.api_msg_status;
                            if (response.print) {
                                form.trigger("reset");
                            ...
                            if($('.receipt_print').prop('checked')) {
                             window.open('../receipt_invoice.php?order_id=' + response.id + '&print=1&frontdesk=1&booking=1', 'mywindow', 'width:1000,height:400 status=1');
                         }
                         var msg = '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">X</button><strong></strong> Order#' + track_no + '  Booked Successfully.</div>';
                         var api_msg = '';
                         if(api_msg_status=='error'){
                            api_msg = '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">X</button><strong>API Order Message: </strong> ' + api_msg + '</div>';
                         }
                         if(api_msg_status=='success'){
                            api_msg = '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">X</button><strong>API Order Message:</strong> ' + api_msg + '</div>';
                         }
                         if(api_msg && api_msg!=''){
                            body.find('.api_msgs').html(msg);
                         }
                         body.find('#msg').html(msg);
                         body.find('.msgs').html(msg);
                         form.trigger("reset");
...
function execute() {
            ...
            function calculateCharges() {
                ...
                var fc_value = parseFloat(deliverCharges / 100 * calculation_fc).toFixed(2);
                ...
                totalcharges = parseFloat(totalcharges) + parseFloat(fc_value);
            ...
            function calculatingNetAmout() {
                ...
                var net_amount = 0;
                net_amount = parseFloat(total_charges);
                // alert(net_amount);
