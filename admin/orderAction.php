...
if (isset($_POST['update_sts'])) {
	...
	$query = mysqli_query($con, "SELECT orders.status,allowed_status,orders.delivery_rider FROM orders LEFT JOIN order_status ON orders.status=order_status.status WHERE  orders.track_no ='$track_no'");
	$record = mysqli_fetch_array($query);
	$allowed_status = explode(',', $record['allowed_status']);
	$check_status  = mysqli_query($con, "SELECT sts_id,marked_done FROM order_status WHERE status ='" . $sts . "'   ");
	$status_record = mysqli_fetch_array($check_status);
	$id_check = $status_record['sts_id'];
	
	$rider_id = isset($status_record['delivery_rider']) ? $status_record['delivery_rider'] :'';
	if (!in_array($id_check, $allowed_status)) {
		...
		if ($can_be_marked_done == 2) {
			...
			$rider_status_done = " rider_status_done_no = '0', ";
		}
		mysqli_query($con, "UPDATE assignment_record SET $rider_status_done status_update_time ='" . $date . "' WHERE order_num = '" . $track_no . "'");
	...
	include "includes/sms_helper.php";
		if ($active_status == "Delivered") {
			updateRiderWalletBalance($track_no, $rider_id);
			$sendSms = sendSmsMobileGateWay($track_no, 'Delivered');
		}else{ 
			$sendSms = sendSmsMobileGateWay($track_no, 'Status Update');
		}	
	...
	$user_id = isset($_SESSION['users_id']) ? $_SESSION['users_id'] : ''; 
	mysqli_query($con, "INSERT INTO order_logs(`order_no` ,`order_status`,`location`,`created_on`,`user_id`) VALUES ('" . $track_no . "' , '" . $sts . "','','" . $date . "','" . $user_id . "') ");
