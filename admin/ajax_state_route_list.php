...
$empQuery = "SELECT country.country_name,country.id as country_id,state.id as state_id,state.state_name FROM state INNER JOIN country on state.country_id=country.id WHERE 1 ".$searchQuery." order by state_id  ".$columnSortOrder." limit ".$row.",".$rowperpage;
// echo $empQuery;
// die;
...
while ($fetch1 = mysqli_fetch_assoc($empRecords)) {
   $data[] = array(
      "id"=>$sr_no++,
      "country"=>'<a href="route_list.php?country_id='.$fetch1['country_id'].'" class="">'.$fetch1['country_name'].'</i></a>',
