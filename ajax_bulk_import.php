...
if (isset($_POST['bulk_booking']) && !empty($_POST['bulk_booking'])) {
	...
	foreach ($getsheet as $row) {
		...
		if (isset($row[0]) && !empty($row[0])) {
			...
			$s = "SELECT id FROM services WHERE service_code = '" . $order_type_str . "' ";
			$service_type_q = mysqli_query($con, $s);
			$order_type_res = mysqli_fetch_array($service_type_q);
			$order_type = $order_type_res['id'];
			$product_type_str = $row[0];
			$q = "SELECT id FROM products WHERE name = '" . $product_type_str . "' ";
			$product_type_q = mysqli_query($con, $q);
			...
			// var_dump($delivery);
			// $delivery = 123;
			// echo "SELECT id FROM services WHERE service_code LIKE '" . $order_type_str . "' ";
			// echo "<br>";
			// echo $origin."<br>";
			// echo $destination."<br>";
			// echo $origin."<br>";
			...
			// echo $delivery."<br>";die();
			if ($delivery <= 0) {
				$err_response = array();
				$err_response['error'] = 1;
				$err_response['alert_msg'] =  "No zone found for Product ".$product_type_str." and Service ".$order_type_str." Origin " . $origin . " and Destination " . $destination;
				echo json_encode($err_response);
				exit();
			}
			// echo '1';
...
foreach ($getsheet as $row) {
	...
	if (isset($row[0]) && !empty($row[0])) {
		...
		$product_type_str = $row[0];
		$product_type_q = mysqli_query($con, "SELECT id FROM products WHERE name = '" . $product_type_str . "' ");
		$product_type_res = mysqli_fetch_array($product_type_q);
		$product_type_id = $product_type_res['id'];
		$order_type_str = $row[1];
		$service_type_q = mysqli_query($con, "SELECT id FROM services WHERE service_code = '" . $order_type_str . "' ");
		$order_type_res = mysqli_fetch_array($service_type_q);
		$order_type = $order_type_res['id'];
		$origin = $row[2];
		$destination = $row[3];
			...
			// echo $insert_qry;
				...
				if ($insert_id > 0) {
					...
					// mysqli_query($con, "UPDATE orders SET net_amount = '" . $net_amount . "',grand_total_charges = '" . $total_charges . "',special_charges = '" . $special_charges . "', pft_amount = '" . $pft_amount . "', fuel_surcharge = " . $fuel_surcharge . " WHERE id = $insert_id ");
							mysqli_query($con, "UPDATE orders SET grand_total_charges = '" . $total_charges . "',special_charges = '" . $special_charges . "', pft_amount = '" . $pft_amount . "', fuel_surcharge = " . $fuel_surcharge . " WHERE id = $insert_id ");
