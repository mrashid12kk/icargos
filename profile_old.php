...
<section class="bg padding30">
  <div class="container-fluid dashboard">
   ...
   $totalcod=0;
    ...
    $querycod= mysqli_fetch_array(mysqli_query($con,"SELECT SUM(collection_amount) AS collection_amount FROM orders WHERE customer_id='".$id."' AND status='Delivered' AND payment_status='Pending'"));
  ...
  $totalorder=0;
    ...
    $querytotal= mysqli_query($con,"SELECT count(id) as customerorderid from orders where customer_id='".$id."'  and status!='Cancelled'");
       while($singleerr = mysqli_fetch_array($querytotal)){

    $totalorder=$singleerr['customerorderid'];
  }

  $main_query = mysqli_query($con,"SELECT os.color_code as color_code,count(o.id) as total_count,os.status as status FROM order_status os LEFT JOIN orders o ON(o.status=os.status AND o.customer_id='".$id."') WHERE 1 GROUP BY os.status ORDER BY os.sort_num ");


  $main_querys = mysqli_query($con,"SELECT os.color_code as color_code,count(o.id) as total_count,os.status as status FROM order_status os LEFT JOIN orders o ON(o.status=os.status AND o.customer_id='".$id."') WHERE 1 GROUP BY os.status ORDER BY os.sort_num ");
 ...
 /*  $query = mysqli_query($con, "SELECT * FROM orders WHERE customer_id = $id");
    ...
    while($single = mysqli_fetch_array($main_query)){
      ...
      ?>
        <div class="col-sm-2 profile-page-title">
          <div style="background: <?php echo $color_code; ?>" class="mini-stat clearfix bx-shadow bg-info dash-item"> <span class="mini-stat-icon"><i class="ion-social-usd"></i></span>
            <div class="mini-stat-info text-right"><a class="whitee" href="orders.php?orderss_status=<?php echo $single['status']; ?>"> <span class=""><?php echo $single['total_count']; ?></span> <?php echo getKeyWordCustomer($id,$single['status']); ?> </a></div>
       ...
       <?php } ?>
        ...
        <div class="col-sm-2 profile-page-title">
          <div class="mini-stat clearfix bx-shadow bg-info6 dash-item" style="background: #0d0150;"> <span class="mini-stat-icon"><i class="ion-social-usd"></i></span>
        ...
        <div class="col-sm-2 profile-page-title">
          <div class="mini-stat clearfix bx-shadow bg-info7 dash-item" style="background: #FF9800;"> <span class="mini-stat-icon"><i class="ion-social-usd"></i></span>
        ...
        <div class="col-sm-2 profile-page-title">
          <div style="background: #FF9800" class="mini-stat clearfix bx-shadow bg-info dash-item"> <span class="mini-stat-icon"><i class="ion-social-usd"></i></span>
  ...
  $query2 = mysqli_query($con,"SELECT SUM(collection_amount) as collection_amount,SUM(net_amount) as grand_total_charges FROM orders WHERE 1 AND status='Delivered' AND payment_status='Pending'  AND customer_id='".$_SESSION['customers']."'");
  ...
  $query3 = mysqli_query($con,"SELECT SUM(net_amount) as grand_total_charges FROM orders WHERE 1 AND status='Returned to Shipper' AND payment_status='Pending'  AND customer_id='".$_SESSION['customers']."'");
            ...
            <div class="panel-body">
              ...
              <div class="row charts" style="background:#fff;border: 1px solid #ccccccab;padding-bottom: 12px;">
                    <div class="col-sm-12">
                      ...
                      {
                          // echo "SELECT count(id) as total_count FROM orders WHERE status='".$single2['status']."'".'<br>';
                      //     die;
                      // $color_code = $single2['color_code'];
                      // $status = $single2['status'];


                        $countorderquery = mysqli_query($con,"SELECT count(id) as total_count FROM orders WHERE status='".$single2['status']."' AND customer_id='".$id."'");
