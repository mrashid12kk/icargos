...
$pickQ = "SELECT assignment_record.order_num as track_no, orders.collection_amount,orders.destination,orders.map_latitude,orders.map_longitude, orders.status,  orders.rname as receiver_name, orders.order_type,  orders.receiver_address, orders.rphone as receiver_phone from assignment_record join orders on assignment_record.order_num = orders.track_no WHERE assignment_record.rider_status_done_no = '0' AND assignment_record.assignment_type=2  AND  assignment_record.user_id =" . $rider_id . "   order by assignment_record.id desc ";
